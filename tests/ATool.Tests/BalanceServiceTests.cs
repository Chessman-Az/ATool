...
public class BalanceServiceTests
{
    ...
    [Fact]
    public async Task RefreshAllAsync_并发两次_两次都执行并各自写入历史()
    {
        ...
        Assert.Equal(2, history.GetByKey(1, from, to).Count); // 两次调用都执行
        Assert.Equal(2, history.GetByKey(2, from, to).Count);
    }
}
